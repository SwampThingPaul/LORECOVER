...
norm_env=function(data){
  ...
  data[data$CY==yr[i]&as.Date(data$Date)%in%leap_dates,6:17]=na.approx(data[data$CY==yr[i]&as.Date(data$Date)%in%leap_dates,6:17])
  }
  }
  #Calculate Penalty
  ...
  data$score=apply(data[, 18:28], 1,FUN=function(x) max(x,na.rm=T))
